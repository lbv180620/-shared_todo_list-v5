...
<body>
    ...
    $active = "top";
    $search = "";
    include_once dirname(__FILE__, 3) . '/components/nav/auth/nav.php';
    ...
    <div class="container">
            ...
            <tbody>
                ...
                <tr <?= $class ?>>
                    ...
                    <td class="align-middle">
                        <a id="item-conf"
                            href="<?= Common::h(DETAIL_PAGE_URL) ?>?item_id=<?= Common::h($item['id']) ?>"><?= Common::h($item['item_name']) ?></a>
                    </td>
                    <!-- 担当者 -->
                    ...
                    <td class="align-middle button">
                        ...
                        <form action="./complete_action.php" method="post" class="my-sm-1">
                            ...
                            <button class="btn btn-primary my-0" type="submit"
                                <?= !is_null($item['finished_date']) ? 'disabled' : '' ?>>完了</button>
                        </form>
                        <?php endif ?>
                        <?php if ($login['is_admin'] === 1 || $login['id'] === $client['id']) : ?>
                        <a href="<?= Common::h(EDIT_PAGE_URL) ?>?item_id=<?= Common::h($item['id']) ?>"
                            class="btn btn-success">修正</a>
                        <a href="<?= Common::h(DELETE_PAGE_URL) ?>?item_id=<?= Common::h($item['id']) ?>"
        ...
        <div class="row my-2">
            ...
            <div class="col-sm-6">
                <form>
                    <div class="goback">
                        <input type="button" value="もどる" class="btn btn-primary my-0"
                            onclick="location.href='<? Common::h(TOP_PAGE_URL) ?>'">
                    </div>
                </form>
            </div>
            <div class="col-sm-3"></div>
